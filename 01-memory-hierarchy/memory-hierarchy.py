...
start_exp = 10 # 2^start_exp will be the starting block size
end_exp   = 25 # 2^end_exp will be the ending block size
...
for exp in xrange(start_exp, end_exp + 1):
    block_size = 2 ** exp
...
w_plt.annotate("$2^{%d}$" % log2(w_minx), xy=(w_minx, w_plt.get_ylim()[0]),
...
r_plt.annotate('$2^{%d}$' % log2(r_minx), xy=(r_minx, r_plt.get_ylim()[1]),
