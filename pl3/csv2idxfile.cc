...
int main(int argc, char **argv) {
    ...
    while (csvf) {
        Record r(indexSchema);

        std::string buf;
        buf.reserve(indexSchema.numAttrs * indexSchema.attrLen);
        ...
        int pos = 0;
        while (stream && pos < indexSchema.numAttrs) {
            char attr[11];
            if (!stream.getline(attr, 11, ','))
                break;
            strcpy((char*)r.at(pos++), attr);
        }

        int slot = add_fixed_len_page(&p, &r, indexSchema);
        added++;
        assert(slot != -1);
